var CSDLoader = function(form) {
    ...
    this.BaseURL = window.location.href.substring(0, window.location.href.indexOf('/api/apps/dhis-ssa-csd-loader'));
...
CSDLoader.prototype.GetKeys = function(key) {
    this.Log('getting keys');
    var keys  = [];
    var url = this.BaseURL + '/api/dataStore/CSD-Loader-Last-' + key;
    var options = {
	url: url,
	method: 'GET',
	type: 'GET',
	...
	error: $.proxy(function() {
	    this.Log('could not fetch last modified time from ' + url);
	},this),
...
CSDLoader.prototype.SetLastModified = function(key,time) {
    var doc = this.form.find('select[name=docs] option:selected').val();
    var method =  ($.inArray(doc,this.GetKeys(key)) >= 0)  ?  'PUT' : 'POST';
    this.Log('setting export to ' + time + ' using ' + method + ' for doc '  + doc);
    var options = {
	url: this.BaseURL + '/api/dataStore/CSD-Loader-Last-' + key + '/' + doc,
	method: method,
...
CSDLoader.prototype.FetchNamespaceValue = function(key) {
    this.Log('getting key ' + key)
    var time = false;
    var doc = this.form.find('select[name=docs] option:selected').val();
    var options = {
	url: this.BaseURL + '/api/dataStore/' + key + '/' + doc,
	method: 'GET',
	type: 'GET',
	dataType: "json",
	async: false,
	...
	cache: false,
	error: $.proxy(function() {
	    this.Log('could not fetch ' + key + '/' + doc);
	},this),
	success: $.proxy(function(json) {
	    //extract time from	<metaData xmlns="http://dhis2.org/schema/dxf/2.0" created="2015-09-25T12:25:52.636+0000">
	    time = json['value'];
...
CSDLoader.prototype.BindActions = function() {
    ...
    var action = this.form.find('input[type=submit]').val();
    if (lm) {
	var lmt = this.FetchNamespaceValue('CSD-Loader-Last-' + action );  
	this.Log('Last  ' + action + ' =  '  + lmt);
	var options = {
	    format:'c',
...
CSDLoader.prototype.LoadDocs = function() {
    ...
    var url = this.BaseILRURL + '/documents.json';
    this.Log('getting docs at ' + url);
    $.ajax({
	url: url,
	contentType: 'application/json',
	dataType: "text",
...
CSDLoader.prototype.Export = function() {
    ...
    $.ajax({
	url: this.BaseURL + '/api/24/metadata',
	method: 'GET',
	type: 'GET',
	context: this,
	data : data,
	...
	success: $.proxy(function(xml) {
		...
		success: $.proxy(function(xml) {
		    ...
		    this.SetLastModified('Export',dhis2time);	
		    this.SetStatusAsLoading(false);
		},this)
	    });
 	},this)
...
CSDLoader.prototype.PollImportStatus = function(that) {
	...
	$.ajax({
		...
		if (Array.isArray(m) && m.length == 0) {
		    UpdateStatus("DHIS2 Import Status: waiting on response"  );
		} else {
...
CSDLoader.prototype.ImportSelected = function() {
    ...
    var lmt = this.form.find('input[name=lastmodified]').val();
    var zip = this.form.find('input[name=zip]').val();
    var record = '';
    this.Log("last modified = " + lmt);
    if (lmt) {
	record = '<csd:record updated="' +lmt + '"/>';
    }
    var msg = 
      "<csd:requestParams xmlns:csd='urn:ihe:iti:csd:2013'>\n"
	...
	if (this.status == 200) {
	    ...
	    var url;
	    if (zip) {
		data = new Blob([this.response],{type:'application/zip'});
		url = that.BaseURL + '/api/24/metadata.xml.zip';
	    } else {
		data = this.response;
		url = that.BaseURL + '/api/24/metadata.xml';
	    }
	    console.log(data)
		...
		success: function(xmlResponse) {
		    ...
		    that.SetStatusAsLoading(false);
		    if (lmt) {
			this.Log('Setting last modified import time to ' + lmt);
			this.SetLastModified('Import',lmt);		    //really should check status of import as part of the ping import 
		    }
		    that.PollImportStatus(that);
		}
